---
name: task-breakdown
description: Break down technical design documents (TDD) into actionable task checklists for the Booking Tour project. Creates granular, implementable tasks grouped by database, backend (NestJS), frontend (Next.js), and testing layers. Use when the user requests task breakdown, task generation, or when using the /tasks command.
---
